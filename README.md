...
这个错误是由以下原因导致的：
1. 使用了测试号（touristappid）进行预览
2. 测试号不支持云开发功能
3. 即使云开发功能已被禁用（USE_CLOUDBASE=false），微信开发者工具仍然会尝试进行安全检查
...
# 使用微信开发者工具打开 dist 目录预览
```

#### 方案 3：使用正式 AppID（完整功能）

如果需要完整功能（云开发排行榜、用户系统等）：

1. 在 project.config.json 中修改 appid 为你的正式 AppID
2. 修改以下两个文件，将 USE_CLOUDBASE 设置为 true：
   - `src/cloudbase/index.ts`
   - `src/pages/loading/index.tsx`
3. 配置云开发环境（详见下文）
4. 重新构建：`pnpm build:weapp`
...
4. 在 `src/cloudbase/index.ts` 中更新环境 ID：
...
5. 在云开发控制台中创建数据库和云函数
6. 上传云函数（使用微信开发者工具或 miniprogram-ci）

详见 `EXPORT_GUIDE.md` 中的详细步骤。

### 云开发功能说明

#### 禁用云开发时（USE_CLOUDBASE=false）
- 排行榜：使用本地存储，数据仅保存在当前设备
- 用户系统：使用本地存储，不支持多设备同步
- 功能限制：无法查看其他玩家的排行

#### 启用云开发时（USE_CLOUDBASE=true）
- 排行榜：使用云数据库，支持跨设备同步
- 用户系统：支持微信登录，数据云端保存
- 完整功能：可查看所有玩家的排行榜
...
A: 这是因为使用了测试号，测试号不支持云开发。可以忽略此错误，游戏会使用本地存储。

### Q: 如何查看排行榜？

A:
- 预览版（USE_CLOUDBASE=false）：排行榜使用本地数据，无法看到其他玩家
- 正式版（USE_CLOUDBASE=true）：使用云数据库，可以看到所有玩家的排行榜

### Q: 如何启用云开发功能？

A: 修改 `src/cloudbase/index.ts` 和 `src/pages/loading/index.tsx` 中的 `USE_CLOUDBASE` 为 true，并配置云开发环境。

### Q: 导出后如何在本地部署？

A: 详见 `IMPORT_GUIDE.md` 中的详细步骤。

## 相关文档

- `EXPORT_GUIDE.md` - 导出部署指南
- `IMPORT_GUIDE.md` - 导入部署指南
- `WECHAT_RELEASE_GUIDE.md` - 微信小程序上传审核指南
- `CLOUDBASE_MIGRATION.md` - 云开发数据迁移指南
- `CHANGELOG.md` - 版本更新历史
- `FIXES.md` - 问题修复日志
```
